...
def logo_goster():
    ...
    print(" " * (len(son) - len(son.lstrip())) + RENK_SARI + "@freebyteV3")
    print(RENK_MAVI + "=" * 60)
    print(RENK_MAVI + "1GB App FreeByte V3".center(60))
    print(RENK_MAVI + "=" * 60)
    ...
    try:
        ...
        if r_start.status_code == 200 and "transactionId" in r_start.json():
            ...
            if r_finish.status_code == 200:
                print(RENK_YESIL + "[✅] Reklam tamamlandı. FreeByte kazanıldı.")
            else:
...
if __name__ == "__main__":
    while True:  # Sonsuz döngü -> program hep başa dönsün
        logo_goster()
        try:
            msisdn = input_renkli("[📱] Telefon numarası (905xxxxxxxxx, Çıkış:q): ")
            if msisdn.lower() == "q":  # Kullanıcı isterse q ile çıkabilsin
                print(RENK_YESIL + "👋 Çıkış yapılıyor...")
                break
            if not msisdn.startswith("905") or len(msisdn) != 12:
                print(RENK_KIRMIZI + "[❌] Geçersiz numara formatı!")
            else:
                manuel_pin_giris(msisdn)
